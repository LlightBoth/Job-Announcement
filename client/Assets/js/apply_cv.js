...
if (applyForm) {
    applyForm.addEventListener('submit', async(e) => {
        ...
        try {
            // fetch from localstorage
            const currentUserStr = localStorage.getItem('save_user');
            const token = localStorage.getItem('user_token');

            if (currentUserStr) {
                const currentUser = await fetch('http://localhost:3000/user/email', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({email: currentUserStr})
                });
                const currentUser_Data = await currentUser.json(); 

                candidateName = currentUser_Data.fullname || candidateName;
                candidateEmail = currentUser_Data.email || candidateEmail;
                candidatePhone = currentUser_Data.phone || candidatePhone;
            }
        } catch (error) {
            console.error(error);
        }
