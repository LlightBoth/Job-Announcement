    ...
    async function renderResetPasswordForm() {
        ...
        if (resetPasswordForm) {
            resetPasswordForm.addEventListener('submit', async(e) => {
                ...
                const user_email = localStorage.getItem('save_user');

                // Fetch from server to change
                const user_res = await fetch('http://localhost:3000/user/edit/password', {
                    method: 'POST',
                    headers: {
                ...
                if (!user_res.ok) {
                    showAlert(user_data.message, "error", "resetAlertContainer");
                    return;
                }

                // Find user
                if (user_data) {
                    showAlert(user_data.message, 'success', 'resetAlertContainer');
